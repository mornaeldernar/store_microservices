...
public static class BasketApiEndpoints
{
        ...
        string customerId, CreateBasketRequest createBasketRequest)
    {
        ...
        var cachedData = await cache.GetStringAsync(createBasketRequest.ProductId);
        if(cachedData is null)
        {
            cachedData = "0";
        }
        var cachedProductPrice = decimal.Parse(cachedData);

        customerBasket.AddBasketProduct(new BasketProduct(createBasketRequest.ProductId,
